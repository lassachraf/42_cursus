...
/*   Updated: 2024/01/30 18:43:59 by alassiqu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	enemy_animation(t_so_long *game)
{
	...
	if (count < 20)
		game->enemy = mlx_xpm_file_to_image(game->mlx,
			"assets/enemy/itachi-1.xpm", &game->img_w, &game->img_h);
	else if (count < 40)
		game->enemy = mlx_xpm_file_to_image(game->mlx,
			"assets/enemy/itachi-2.xpm", &game->img_w, &game->img_h);
	else if (count < 60)
		game->enemy = mlx_xpm_file_to_image(game->mlx,
			"assets/enemy/itachi-3.xpm", &game->img_w, &game->img_h);
	else if (count < 80)
		game->enemy = mlx_xpm_file_to_image(game->mlx,
			"assets/enemy/itachi-4.xpm", &game->img_w, &game->img_h);
	else if (count < 100)
		game->enemy = mlx_xpm_file_to_image(game->mlx,
			"assets/enemy/itachi-5.xpm", &game->img_w, &game->img_h);
	if (count == 100)
		count = 0;
	put_enemy(game);
	count++;
